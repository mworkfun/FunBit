...
//% groups="['Motor', 'RGB-led', 'Sensor', 'Laser']"
namespace funBit {
    /**
   ...
   /////////////////////////////////////////////////////
     ...
     * @param degree, eg: 90
     */
    //% block=" Laser $Dir| degree:%degree"
    //% degree.min=0 degree.max=90
    //% group="Laser" weight=66
    export function SetLaser(Dir: LaserDir, degree: number) {
        if (!PCA9685_Initialized) {
            init_PCA9685();
        }
        ...
        if (Dir == 1) {    
            degree = Math.map(degree, 0, 90, 90, 180);
     ...
     */
    //% block="Laser $sta"
    //% group="Laser" weight=65
    export function OnOff(sta: OnOff) {
        if (!PCA9685_Initialized) {
            init_PCA9685();
        }
